...
plot(data[1:N,])
plot.ts(
  ts(data[1:N, "NO"], start=1, frequency=6),
  col=data[1:N, "cluster"],
)

# 4-layers neural network
#net <- nnet(cluster ~ NO+NO2+NOX+WDIR, data=data[1:N,], size=8)
net <- monmlp.fit(x=as.matrix(data[1:N,1:4]), y=as.matrix(as.numeric(data[1:N, 5])), 
                  hidden1=5, hidden2=5, n.trials=10, n.ensemble=4, bag=T)
